...
* We could run this on our full set of 113 red + black spruce samples, but the results would be dominated by loci that are strongly divergent between the species (potentially owing to reproductive isolation).  What we'd like to find is evidence of adaptive introgression, where alleles from black spruce have contributed to divergence and local adaptation *within* red spruce
...
* If so, we can have PCAngsd run a selection scan to identify loci under selection along 1 or more of the PC axes it identifies within red spruce.  Knowing which PC axis relates to introgression should then let us test outlier loci within red spruce due to introgression from its sister species.
...
After we've got these results files, we can go over to **RStudio** and start exploring the results.
...
p1 <- ggscatter(data2, x ="qBS", y = "V1",
          color="Pop",
          main="Red spruce genetic PC1 v. % ancestry from black spruce") +
  theme_bw(base_size = 13, base_family = "Times") +
  labs(x = "Black spruce ancestry", y = "PC1 score")

p2 <- ggscatter(data2, x ="qBS", y = "V2",
                color="Pop",
                main="Red spruce genetic PC2 v. % ancestry from black spruce") +
  theme_bw(base_size = 13, base_family = "Times") +
...
```
plot(-log10(pval_PC1$p_PC1[2e05:2.01e05]),
  col=as.factor(pos_filtered$chromo), 
  xlab="Position", 
  ylab="-log10(p-value)", 
...
# We can export these outlier lists out to text files for future use
...
# We can also get info on the # of unique contigs containing selection outliers
...
# There may be some black spruce loci coming in from introgression that are fueling the adaptive divergence between the red spruce pops along PC2 (latitudinal divergence). To look for this, we can calculate overlap between the selection outliers on PC1 and outliers on PC2.

```
library(ggVennDiagram)

outliers_venn = list(outlier_contigs_PC1, outlier_contigs_PC2)

# And make the Venn Diagram
