...
# Merge the four dataframes from each contrast giving each column a unique suffix associated with the contrast
res_list <- list(resG1_CvT, resG2_CvT, resG3_CvT, resG4_CvT)

res_gen_df <- Reduce(function(x, y) merge(x, y, by = "row.names"), res_list)
rownames(res_gen_df) <- res_gen_df$Row.names
res_gen_df$Row.names <- NULL
...
# Define color mapping logic with mutate based on significance and up and down regulation, or sig. in both for just G1 and G2
...
plot1v2 <- ggplot(res_gen_df, aes(x = log2FoldChange.G1, y = log2FoldChange.G2, color = fill)) +
  ...
  labs(x = "Log2 Fold Change 28 vs. 18", 
       ...
       title = "How do significantly differentially expressed genes vary between Gen 1 vs. Gen 2?") +
  theme_minimal() 



### You can expand the code above to define color mapping for different contrasts (e.g., generation contrasts)
