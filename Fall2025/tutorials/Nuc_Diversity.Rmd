...
Total number of sites sequenced

```{r}
sum(theta$nSites) # number of sites sequenced
```

We can remove any windows with few sites (<100) to avoid any biases in nucleotide diversity from small sample sizes
...
And plot as a histogram of nSites
...
ggplot(theta2, aes(x=nSites)) +
  ...
  labs(title = "Distribution of sites with data", x = "Number of sites in 50kb windows", y = "Frequency")
```
