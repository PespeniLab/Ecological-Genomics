...
1. Understand the Gene Ontology (GO) and functional enrichment statistical tests.
...
### First, what do the distributions of our results look like?
...
# Filter DEGs that have GO annotations
...
#do the same thing to look at gen1, then you alter the tonsa_stat object and run from below

# DESeq2 results object for 1st generation
res_G1_int <- results(dds_interaction, name = "line_treatment_vs_control")

# Convert to data frame
res_G1_df <- as.data.frame(res_G1_int)
...
filtered_GO_results <- res_table %>%
  filter(Annotated >= 1 & Annotated < 250) %>%  # adjust as needed
  group_by(Term) %>%
