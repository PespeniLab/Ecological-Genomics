...
# Counts of specific top interaction gene! (important validatition that the normalization, model is working)
d <-plotCounts(dds, gene="TRINITY_DN30_c0_g2::TRINITY_DN30_c0_g2_i1::g.130::m.130", intgroup = (c("treatment","generation")), returnData=TRUE)
d
...
summary(res_F0_AMvOW)
res_F0_AMvOW <- res_F0_AMvOW[!is.na(res_F0_AMvOW$padj),]
degs_F0_AMvOW <- row.names(res_F0_AMvOW[res_F0_AMvOW$padj < 0.05,])

# For AM vs OA
res_F0_AMvOA <- results(dds_sub, name="treatment_OA_vs_AM", alpha=0.05)
res_F0_AMvOA <- res_F0_AMvOA[order(res_F0_AMvOA$padj),]
head(res_F0_AMvOA)

summary(res_F0_AMvOA)
res_F0_AMvOA <- res_F0_AMvOA[!is.na(res_F0_AMvOA$padj),]
degs_F0_AMvOA <- row.names(res_F0_AMvOA[res_F0_AMvOA$padj < 0.05,])

# For AM vs OWA
res_F0_AMvOWA <- results(dds_sub, name="treatment_OWA_vs_AM", alpha=0.05)
res_F0_AMvOWA <- res_F0_AMvOWA[order(res_F0_AMvOWA$padj),]
head(res_F0_AMvOWA)

summary(res_F0_AMvOWA)
res_F0_AMvOWA <- res_F0_AMvOWA[!is.na(res_F0_AMvOWA$padj),]
degs_F0_AMvOWA <- row.names(res_F0_AMvOWA[res_F0_AMvOWA$padj < 0.05,])

library(eulerr)

# Total
length(degs_F0_AMvOA)  # 520
length(degs_F0_AMvOW)  # 4841 
length(degs_F0_AMvOWA)  # 3742

# Intersections
length(intersect(degs_F0_AMvOA,degs_F0_AMvOW))  # 387
length(intersect(degs_F0_AMvOA,degs_F0_AMvOWA))  # 340
length(intersect(degs_F0_AMvOWA,degs_F0_AMvOW))  # 2585

intWA <- intersect(degs_F0_AMvOA,degs_F0_AMvOW)
length(intersect(degs_F0_AMvOWA,intWA)) # 308
